...
mod benchmarks {
	...
	#[benchmark]
	fn verify_entry(x: Linear<100, 2000>) -> Result<(), BenchmarkError> {
	...
	#[benchmark]
	fn latest_relay_block() -> Result<(), BenchmarkError> {
		let keys = fill_relay_storage_roots::<T>();

		let result;
		#[block]
		{
			result = get_latest_relay_block::<T>();
		}

		assert_eq!(
			result, keys.last().cloned().expect("There should be at least one key")
		);
		Ok(())
	}

	#[benchmark]
	fn p256_verify() -> Result<(), BenchmarkError> {
		let input = vec![
